			...
			It("should work with custom ClusterBoostrap", func() {
				...
				By("filling custom ClusterBootstrap correctly", func() {
					Eventually(func(g Gomega) {
						...
						g.Expect(clusterBootstrap.Spec.CNI.RefName).To(BeEquivalentTo("calico.tanzu.vmware.com.1.2.5--vmware.1-tkg.1"))
						...
						// Validate Kapp
						g.Expect(clusterBootstrap.Spec.Kapp.RefName).To(Equal("kapp-controller.tanzu.vmware.com.0.31.9"))
						...
						g.Expect(clusterBootstrap.Spec.AdditionalPackages[0].RefName).To(BeEquivalentTo("pinniped.tanzu.vmware.com.0.12.1--vmware.1-tkg.1"))
						...
						g.Expect(clusterBootstrap.Spec.AdditionalPackages[1].RefName).To(BeEquivalentTo("foobar.tanzu.vmware.com.0.1.2--vmware.1-tkg.1"))
				...
				By("upgrading Cluster to newer TKR", func() {
					...
					Eventually(func(g Gomega) {
						...
						cni := upgradedClusterBootstrap.Spec.CNI
						// Note: The value of CNI has been bumped to the one in TKR after cluster upgrade
						g.Expect(cni.RefName).To(Equal("calico.tanzu.vmware.com.1.2.6--vmware.1-tkg.1"))
						...
						// Validate Kapp
						kapp := upgradedClusterBootstrap.Spec.Kapp
						g.Expect(kapp.RefName).To(Equal("kapp-controller.tanzu.vmware.com.0.31.10"))
						...
						g.Expect(upgradedClusterBootstrap.Spec.AdditionalPackages[0].RefName).To(BeEquivalentTo("pinniped.tanzu.vmware.com.0.12.2--vmware.1-tkg.1"))
						...
						g.Expect(upgradedClusterBootstrap.Spec.AdditionalPackages[1].RefName).To(BeEquivalentTo("foobar.tanzu.vmware.com.0.1.3--vmware.1-tkg.1"))
