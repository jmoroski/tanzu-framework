...
func setCLIRepository(node *yaml.Node, repository configapi.PluginRepository) (persist bool, err error) {
	...
	return setRepository(repositoriesNode, repository, nodeutils.WithPatchStrategyKey(fmt.Sprintf("%v.%v.%v", KeyClientOptions, KeyCLI, KeyRepositories)), nodeutils.WithPatchStrategies(patchStrategies))
}
