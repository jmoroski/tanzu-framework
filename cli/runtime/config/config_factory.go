...
type CfgPersistOptions struct {
	CfgPath string
}

type CfgPersistOpts func(config *CfgPersistOptions)

func WithCfgPath(path string) CfgPersistOpts {
	return func(config *CfgPersistOptions) {
		config.CfgPath = path
	}
}

// persistNode stores/writes the yaml node to config.yaml
func persistNode(node *yaml.Node, opts ...CfgPersistOpts) error {
	configurations := &CfgPersistOptions{}
